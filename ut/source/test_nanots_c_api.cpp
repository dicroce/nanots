...
void test_nanots_c_api::test_c_api_basic_write_read() {
  ...
  nanots_result_t result = nanots_writer_write(
...
void test_nanots_c_api::test_c_api_iterator_functionality() {
  ...
  nanots_result_t result = nanots_iterator_find(iterator, 1200);
  RTF_ASSERT(result == NANOTS_OK);
  RTF_ASSERT(nanots_iterator_valid(iterator) == 1);
...
void test_nanots_c_api::test_c_api_contiguous_segments() {
  ...
  nanots_result_t result = nanots_reader_query_contiguous_segments(
...
void test_nanots_c_api::test_c_api_error_handling() {
  ...
  RTF_ASSERT(nanots_iterator_get_current_frame(nullptr, &frame_info) ==
             NANOTS_INVALID_HANDLE);
  RTF_ASSERT(nanots_iterator_next(nullptr) == NANOTS_INVALID_HANDLE);
  RTF_ASSERT(nanots_iterator_prev(nullptr) == NANOTS_INVALID_HANDLE);
  RTF_ASSERT(nanots_iterator_find(nullptr, 1000) == NANOTS_INVALID_HANDLE);
  RTF_ASSERT(nanots_iterator_reset(nullptr) == NANOTS_INVALID_HANDLE);
  ...
  result = nanots_writer_write(writer, context, (const uint8_t*)data,
                               strlen(data), 1000, 0);
  RTF_ASSERT(result == NANOTS_INVALID_TIMESTAMP);
